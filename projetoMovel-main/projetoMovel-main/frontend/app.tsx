...
const LoginScreen = ({ onLogin }) => {
    ...
    const handleLogin = () => {
        // Usamos alert aqui porque estamos simulando um ambiente React Native
        ...
        //     alert('E-mail ou senha incorretos! Use teste@mail.com / 12345');
        // }
        // Para fins de demonstração, sempre faz login com o email digitado:
        onLogin(email || 'teste@mail.com');
    };
...
const Dashboard = ({
    ...
    // ESTADO DE PERFIL ATUALIZADO com todos os campos solicitados
    const [perfil, setPerfil] = useState({
        nome: 'João Silva',
        email: email, // Valor inicial é o email de login
    ...
    useEffect(() => {
        ...
        for (let h = 0; h < 24; h++) {
            ...
            const umidade = 40 + Math.random() * 30;
            simular.push({
    ...
    const textos = {
        ...
        en: {
            ...
            nome: 'Full Name',
            email: 'Access Email',
            nomeFazenda: 'Farm Name',
            endereco: 'Address (Street, Number)',
        ...
        es: {
            ...
            nomeFazenda: 'Nombre de la Granja',
            endereco: 'Dirección (Calle, Número)',
            cidade: 'Ciudad y Estado',
        },
    };
    ...
    const renderHome = () => (
        <ScrollView style={{ marginBottom: 80 }}>
            ...
            {dados.map((d, i) => (
                <View key={i} style={[styles.dataCard, { backgroundColor: cores.card }]}>
                    <Text style={{ color: cores.texto, fontSize: fonteBase }}>
                        🕒 {d.hora} | 🌤️ {d.temperatura}°C | 💧 {d.umidade}%
                    </Text>
                </View>
            ))}
            ...
            <View style={[styles.dataCard, { backgroundColor: cores.card }]}>
                <Text style={{ color: cores.texto, fontSize: fonteBase }}>
                    Média Temp: {media(dados, 'temperatura')}°C
                </Text>
                <Text style={{ color: cores.texto, fontSize: fonteBase }}>
                    Média Umid: {media(dados, 'umidade')}%
                </Text>
            </View>
        </ScrollView>
    );
    ...
    const renderPerfil = () => {
        // Define a ordem e as chaves dos campos do perfil
        const fieldKeys = [
            'nome',
            'email',
            'nomeFazenda',
                ...
                {fieldKeys.map((key) => {
                    ...
                    return (
                        <View key={key} style={{ marginBottom: 15 }}>
                            <Text style={{ color: cores.texto, fontSize: fonteBase - 1, marginBottom: 5, fontWeight: '600' }}>
                                {t[key]}
                            </Text>
                            <TextInput
                                placeholder={t[key]}
                                style={[
                                    styles.input,
                                    {
                                        ...
                                        color: cores.texto,
                                        fontSize: fonteBase,
                                        width: '100%', // Usar 100% de largura
                                        marginVertical: 0,
                                        opacity: isEmail ? 0.6 : 1, // Visualmente desabilita o email
                                    },
                                ]}
                                keyboardType={isEmail ? 'email-address' : 'default'}
                                value={perfil[key]}
                                onChangeText={(txt) => setPerfil({ ...perfil, [key]: txt })}
                                editable={!isEmail} // O campo email é somente leitura
                            />
                        </View>
                    );
                })}

                <TouchableOpacity style={[styles.button, { backgroundColor: cores.destaque, width: '100%' }]}>
                    <Text style={{ color: '#fff', textAlign: 'center', fontWeight: 'bold', fontSize: fonteBase }}>
                        💾 {t.salvar}
                    </Text>
                </TouchableOpacity>
            </ScrollView>
    ...
    return (
        <View style={[styles.dashboard, { backgroundColor: cores.fundo }]}>
            ...
            <View style={styles.tabBar}>
                {['home', 'perfil', 'configuracao'].map((name) => (
                    <TouchableOpacity
                        key={name}
                        onPress={() => setTab(name)}
                        style={[
...
// FUNÇÕES AUXILIARES
// =====================================================
const media = (dados, campo) =>
    (dados.reduce((acc, d) => acc + parseFloat(d[campo]), 0) / dados.length).toFixed(1);
...
const styles = StyleSheet.create({
    ...
    // Ajustei o estilo input para remover a largura fixa
